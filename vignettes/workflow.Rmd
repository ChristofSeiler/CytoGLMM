...
Install [R](https://www.r-project.org/) and [RStudio](https://www.rstudio.com/). Open this `Rmd` file in Rstudio. Then run the following code code to install all required packages.
...
Set style and computational parameters.
...
Reanalysis of mass cytometry data from [@aghaeepour2017immune]. The `fcs` files can be downloaded from [FlowRepository](http://flowrepository.org/id/FR-FCM-ZY3Q). We use the package `FlowRepositoryR` to download `fcs` files. We download mass cytometry data measured on whole blood samples collected from 16 women during pregnancy stimulated with IFNa at first and third trimester.
...
The previous commands downloaded `fcs` files to the `FR-FCM-ZY3Q` folder. Now, we prepare sample table by parsing the `fcs` filenames. For some studies, it might be easier to prepare `sample_table.csv` and load it using the `readr` package.
...
Load marker isotopes and protein names. Make sure that maker names don't have `-` or start with a number. This is important to be compatible with the formula syntax of `R`.
...
Define the columns in `df_samples` that are not protein markers. In our example, we have donor ID, `FCS` filename, if the cell was collected during the first or second trimester, and the cell types that we have just defined through gating.
...
NK cell count. List the smallest and largest term and donor combinations.
...
Fit a Generalized Linear Mixed Models (GLMM) with donor random effects.
...
Instead of modeling the donor effect, we can use bootstrap resampling. This also an option if we do not have samples repeated (under different conditions) from the same donor. This function is a wrapper around the package `mbest` [@perry2017fast].
...
Fit a mixture of regression model to identity if there are clusters of donors. This function is a wrapper around the package `flexmix` [@grun2007fitting].
...
The plotting function automatically uses the BIC criterion to select the number of clusters.
