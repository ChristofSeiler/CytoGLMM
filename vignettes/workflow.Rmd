---
title: "Reanalysis of CyTOF Data from Aghaeepour 2017"
author:
...
This is a step-by-step guide to illustrated a complete analysis of mass cytometry data.

# Prerequisites

Install [R](https://www.r-project.org/) (currently testes on R 3.4 and higher) and [RStudio](https://www.rstudio.com/). Open this `Rmd` file in Rstudio. Then run the follwing code code to install all required packages.

```{r install_packages, warning=FALSE, message=FALSE, eval=FALSE}
pkgs_needed = c("devtools","tidyverse","readr","magrittr","stringr",
                "reshape2",
                "flowCore","openCyto","ggcyto","FlowRepositoryR",
                "scales","parallel")
...
Reanalysis of mass cytometry data from *An immune clock of human pregnancy* by Aghaeepour and co-authors published in 2017. The `fcs` files can be downloaded from [http://flowrepository.org/id/FR-FCM-ZY3Q](http://flowrepository.org/id/FR-FCM-ZY3Q). We use the package `FlowRepositoryR` to download `fcs` files. We download mass cytometry data measured on whole blood samples collected from 16 women during pregnancy stimulated with IFNa at first and third trimester.
...
We downloaded the `fcs` files to the `FR-FCM-ZY3Q` folder. Prepare sample table.
...
sample_table = tibble(
  ...
  treatment = rep("IFNa",length(fcs_files)),
...
Load marker isotopes and protein names. Make sure that maker names don't havw `-` or start with a number. This is important to be compatible with the forumla syntax of `R`.
...
Gate with `openCyto` according to supplementary material (Fig. S1).
...
MDS on median marker expression on original.

```{r plot_mds, fig.wide=TRUE}
...
PCA plot of all cells color coded by cell type.

```{r plot_prcomp, fig.wide=TRUE}
...
# Analysis of NK Cells Subset

We will now focus only on the NK cells.
...
Instead of modeling the donor effect, we use bootstrap resampling. This can also be used if we do not have samples from the same donor under different conditions.
...
mix_fit = CytoGLMM::cytoflexmix(df_samples_subset, 
                                protein_names = protein_names,
                                condition = "term", group = "donor",
                                cell_n_min = 1000, cell_n_subsample = 1000, 
                                ks = 1:nlevels(df_samples_subset$donor))
plot(mix_fit)
```
...
In this case, it would also make sense to select only seven clusters, as there is not much improvements with seven or more clusters.
