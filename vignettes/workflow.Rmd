...
pkgs_needed = c("devtools","tidyverse","magrittr",
                ...
                "ggcorrplot","SummarizedExperiment","lme4","lmerTest",
                "broom")
...
Predict term from all median marker expressions for NK cells.

```{r term_prediction}
df_median_subset = df_median %>% dplyr::filter(celltype == "NK")
prot_coll = paste(protein_names, collapse = " + ")
formula_str = paste0("term ~ ", prot_coll, " + (1|donor)")
fit_glmer = glmer(formula_str, family = binomial, data = df_median_subset)
```
