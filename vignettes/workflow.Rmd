...
We bin expressions into discrete categorical variables to handle multimodal marker expression profiles.
...
```{r}
subids = sample(nrow(df_samples),10000)
df_subsamples = df_samples_binned[subids,]
table(df_subsamples$donor)
fit_vb = lcr(df_subsamples,
             protein_names,
             condition = "condition",
