...
```{r read_sample_table}
path = find_package_root_file("vignettes", "sample_table.csv")
sample_table = read.csv(path)
...
sample_list = lapply(sample_table$file_name,function(file_name_fcs) {
  path = find_package_root_file("vignettes", file_name_fcs)
  fcsB = read.FCS(path,transformation = FALSE)
...
```{r map_protein_names}
path = find_package_root_file("vignettes", "markers.csv")
markers = read.csv(path,stringsAsFactors = FALSE)
...
In Latent Class Regression (LCR) approach, we addresses the potential multimodal marker distributions by binning the data prior to analysis.
...
ggobj = plot_pairwise_mi(fit,
                       df_samples,
                       protein_names)
ggsave(ggobj,
       filename = paste0("pairwise_mi_seed_",seed,"_subsample_",subsample,"_R",num_latent_classes,"_normal.pdf"),
       height = 5, width = 9)
```
...
```{r fig.height=4,fig.width=5}
ggobj = plot_pairwise_mi_da(fit,
                            df_samples,
                            protein_names)
ggobj
ggsave(ggobj,
       filename = paste0("pairwise_mi_da_seed_",seed,"_subsample_",subsample,"_R",num_latent_classes,"_normal.pdf"),
       height = 5, width = 6)
```

Choosing the number of latent classes using leave-one-participant-out cross-validation.

```{r}
# TODO
```

Such a nonparametric modeling approach removes the distributional assumption and instead requires the user to choose an appropriate number bins. By choosing small number of bins, we risk missing some local features in the marker expressions, e.g. fusing two modes together. By choosing large number of bins, we increase the computation time and posterior variability due to the increase number of parameter that have to be estimated. To investigate the sensitive of our results to bins size, we examined a range of different bins sizes.
