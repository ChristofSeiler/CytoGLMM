...
```{r eval=FALSE}
set.seed(1234)
protein_subset = pvalues(res)$protein_names[ pvalues(res)$pvalues_adj < 0.01 ] %>% 
  as.character
subsample_ids = sample(x = nrow(df_samples),
                       size = 10000,
                       replace = FALSE)
df_samples_subset = df_samples[subsample_ids,]
res_pca = prcomp(df_samples_subset[,protein_subset],scale. = FALSE)
screeplot(res_pca)
explained_var = (100*res_pca$sdev^2/sum(res_pca$sdev^2)) %>% round(.,1)
plot_samples = function(color_var) {
  autoplot(res_pca, 
